...
Poly TD는 워크래프트 유즈맵식 랜덤 디펜스의 흐름을 싱글 플레이 로그라이크 구조로 옮긴 게임이다. 플레이어는 전투 중 유닛을 무작위로 뽑고, 같은 유닛 3개 또는 지정 레시피를 조합해 상위 영웅으로 올린다. 각 스테이지는 웨이브 단위로 진행되며, 보스 처치와 미션 달성을 통해 재화나 보상을 얻고 다음 스테이지로 넘어간다.
...
- 이후 다시 스테이지 선택 화면으로 돌아가거나 다음 맵으로 진행한다.
...
Poly TD의 플레이 감각은 `랜덤 소환 -> 같은 유닛 3개 조합 -> 레시피 조합 -> 고급 영웅 확보 -> 웨이브/보스 돌파 -> 다음 스테이지 선택`으로 압축된다. UI는 이 루프를 빠르게 반복하도록 설계되어 있으며, 특히 하단 유닛 상세 패널과 우측 조합/판매 버튼이 핵심 조작 지점이다.
