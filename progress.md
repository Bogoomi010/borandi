...
- Manual play legend metadata is now consistency-checked: `yarn manual-playlog` and `yarn balance-audit` reject sessions where `legends` is positive but `maxGrade` is below legend.
