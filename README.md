...
`yarn manual-playlog`는 사람이 직접 플레이한 세션을 `output/manual-balance-playlog.json`에 누적한다. `--source` 기본값은 `human-playtest`이고, 이 출처만 `사람이 직접 2시간 플레이`와 수동 목표 결과 행에 반영된다. `--source=codex-direct-playtest`는 Codex가 브라우저를 직접 조작한 보조 증거로 남기기 위한 값이며, `summary`/`balance-audit`에서 별도 진단으로 표시되지만 사람 120분 증거를 대체하지 않는다. `balance-audit`의 `Codex 직접 조작 보조 증거 분리` 행은 이 보조 세션이 human 집계에서 제외되었는지 보여준다.
