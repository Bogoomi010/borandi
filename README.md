...
yarn manual-playlog --difficulty=normal --minutes=24 --result=loss --stage=1 --round=39 --seed=RUN123 --legends=1 --maxGrade=legend --dataVersion=RESULT_DATA_VERSION --stateChecksum=RESULT_CHECKSUM --inputCount=RESULT_INPUT_COUNT --notes="후반 누적 압박"
                          # 사람이 직접 플레이한 수동 밸런스 세션을 output/manual-balance-playlog.json에 누적
yarn manual-playlog --difficulty=normal --minutes=24 --result=loss --stage=1 --round=39 --seed=RUN123 --legends=1 --maxGrade=legend --dataVersion=RESULT_DATA_VERSION --stateChecksum=RESULT_CHECKSUM --inputCount=RESULT_INPUT_COUNT --source=codex-direct-playtest --notes="Codex 브라우저 직접 조작"
                          # Codex가 브라우저를 직접 조작한 보조 세션으로 누적한다. 사람 120분 증거에는 포함되지 않는다.
yarn manual-playlog --difficulty=normal --minutes=24 --result=loss --stage=1 --round=39 --seed=RUN123 --legends=1 --maxGrade=legend --dataVersion=RESULT_DATA_VERSION --stateChecksum=RESULT_CHECKSUM --inputCount=RESULT_INPUT_COUNT --notes="후반 누적 압박" --dry-run
...
yarn manual-playlog --finish=RUN123 --result=loss --round=40 --legends=1 --maxGrade=legend --dataVersion=RESULT_DATA_VERSION --stateChecksum=RESULT_CHECKSUM --inputCount=RESULT_INPUT_COUNT --endedAt=RESULT_ENDED_AT
                          # 시작 마커의 난이도/맵/시드/시작 시각으로 결과 세션 마무리
yarn manual-playlog --finish-latest --result=loss --round=40 --legends=1 --maxGrade=legend --dataVersion=RESULT_DATA_VERSION --stateChecksum=RESULT_CHECKSUM --inputCount=RESULT_INPUT_COUNT --endedAt=RESULT_ENDED_AT
