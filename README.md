...
yarn manual-playlog --start-next --seed=RUN123
                          # 다음 필요 수동 세션의 난이도/목표로 시작 마커 바로 저장
...
결과 화면의 `기록+다음 복사` 버튼은 이번 결과를 기록한 뒤 바로 `yarn manual-playlog --next`까지 실행하는 명령을 복사한다. `yarn manual-playlog --next`와 `--plan`은 다음 세션의 시작 마커 검증 명령, 실제 시작 마커 명령 템플릿, 마감 조건을 함께 보여주며, `--next-json`/`--plan-json`도 같은 `finishTemplate`과 `startNextDryRunCommandTemplate`을 제공한다. 단, finish되지 않은 pending 시작 마커가 있으면 `--summary`와 `--next`는 새 `--start-next` 추천을 숨기고 기존 마커의 저장 전 `--dry-run` finish 명령과 실제 finish 명령을 먼저 보여준다. `--next-json`도 이때 `blockedByPendingStartMarkers: true`와 `next: null`을 반환한다. `yarn manual-playlog --start-next --seed=...`는 다음 필요 세션의 난이도와 목표 메모를 자동으로 채워 시작 마커를 저장하고, 목표 조건에 맞는 finish 명령 예시를 출력한다.
