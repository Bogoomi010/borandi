...
`yarn manual-playlog --pending`과 `--summary`는 시작 마커별 경과 시간과 12분 목표까지 남은 시간을 보여준다. 미완료 시작 마커의 finish 안내는 저장 전 `--dry-run` 검증 명령을 먼저 보여주며, `yarn manual-playlog --assert` 실패 출력과 `yarn balance-audit`의 미완료 수동 증거 행도 `yarn manual-playlog --preflight`, `yarn manual-playlog --plan`, 추천 시작 검증, 실제 시작 마커를 함께 출력한다.
