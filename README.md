...
yarn manual-playlog --difficulty=normal --minutes=24 --result=loss --stage=1 --round=39 --legends=1 --notes="후반 누적 압박"
...
`yarn balance-audit`는 `browser-direct` JSON의 시나리오 범위와 관찰 게이트도 확인해 실제 브라우저 입력 표본이 난이도별 목표와 같은 방향인지 검사한다.
수동 플레이 로그는 `docs/manual-balance-playlog.example.json` 형식을 따른다. `yarn balance-audit`는 총 120분 이상, 5개 난이도 포함, 입문자 40R 무전설 클리어, 일반 40R 1~2전설 클리어, 중급자 40R 5전설 이상 클리어, 고수 40R 5전설 이하 실패 + 40R 6전설 이상 클리어, 초고수 실패 기록을 확인한다.
