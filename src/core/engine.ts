...
export class Game {
  ...
  private fireAt(
    ...
    const s = this.state;
    const atkSpeed = d.attackSpeed * (d.family === "storm" ? 1 + 0.1 * lv.storm : 1) * this.buffMult(u, "attackSpeed");
    u.cooldown = 1 / atkSpeed;

    let atk = d.attack * this.legendCommandAttackMult() * (d.family === "flame" ? 1 + 0.12 * lv.flame : 1) * this.buffMult(u, "attack");
